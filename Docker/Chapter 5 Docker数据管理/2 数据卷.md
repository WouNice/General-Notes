...
数据卷的设计目的就是数据的持久化，因为其生存周期独立于容器的生存周期，因此 Docker 不会在容器删除时删除其挂载的数据卷。
...
通过`docker volume help`可以查看 Volume 有哪些命令。
...
通过`docker volume COMMAND --help`可以查看每个 Volume 命令的详情。

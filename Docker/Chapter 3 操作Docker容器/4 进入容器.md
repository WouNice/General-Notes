...
在使用-d 参数时，容器启动后会进入后台，用户无法看到容器中的信息，也无法进行操作。这个时候如果需要进入容器进行操作，可以使用 `docker attach` 命令或 `docker exec` 命令，但推荐使用 exec 命令。
...
然而 attach 命令有缺陷：当多个窗口同时 attach 到同一个容器的时候，所有窗口都会同步显示；当某个窗口因命令阻塞时，其他窗口也会无法执行操作。

## exec命令

从 Docker 的 1.3.0 版本起，Docker 提供了一个更加方便的工具 exec 命令，可以在运行中容器内直接执行任意命令。通过 exec 命令对容器执行操作是最为推荐的方式。

...
`docker [container] create`命令支持的选项主要包括如下几大类：与容器运行模式相关、与容器环境配置相关、与容器资源限制和安全保护相关，参见表1～表3。

表1 create命令与容器运行模式相关的选项
...
| --group-add=[]                                               | 运行容器的用户组                                             |
| -i，-- interactive=`true|true`                               | 保持标准输人打开，默认为false                                |
...
| --net-alias=[]                                               | 容器在网络中的别名                                           |
| -P，--publish-all=`true|true`                                | 通过NAT机制将容器标记暴露的端口自动映射到本地主机的临时端口  |
...
| -- rm=`true|true`                                            | 容器退出后是否自动删除，不能跟-d同时使用                     |
| -t，--tty=`true|true`                                        | 是否分配一个伪终端，默认为false                              |
| -- tmpfs=[]                                                  | 挂载临时文件系统到容器                                       |
...
表2 create命令与容器环境和配置相关的选项

| 选项                        | 说明                                                         |
| --------------------------- | ------------------------------------------------------------ |
| --add-host=[]               | 在容器内添加一个主机名到IP地址的映射关 系(通过/etc/hosts 文件) |
...
使用`docker [container] start`命令来启动一个已经创建的容器但处于终止状态的容器。例如，启动刚创建的ubuntu容器：
...
如果需要让Docker容器在后台以守护态（Daemonized）形式运行，可以通过添加-d参数来实现。

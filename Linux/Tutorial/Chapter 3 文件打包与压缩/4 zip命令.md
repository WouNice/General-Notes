...
zip 命令用于解压缩后缀名为.zip 的归档文件。该命令可以将多个文件打包成一个压缩包，也可以在解压缩时指定路径名，而 gzip 和 bzip2 命令是无法指定路径的，对于目录也是无法压缩的。
...
不解压查看 zip 压缩包中的文件信息（不是查看文件内容），格式如下：
...
```shell
# 将/opt 目录下的 1.txt 和 2.txt 文件打包压缩到/home 目录下生成 test.zip 压缩文件，此处的操作目录可以不用固定在/opt 下，都是根据绝对路径实现的
# 第一步：查看/home 和/opt 目录下的文件信息，并查看文件 1.txt 的内容
...
# 第二步：使用 zip 命令按照需求进行压缩，查看/opt 目录，可以发现原文件是存在的
...
# 使用 unzip 命令将/home/test.zip 文件通过“-d”选项解压到/mnt 目录下，并查看/home 目录下的 zip 文件是否存在，确认/mnt 目录下的 1.txt 文件内容是否解压出错
